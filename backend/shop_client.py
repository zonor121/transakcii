...
# Патч: принудительно отключаем HTTP/2 для стабильности на Windows
...
class FlowerShopClient:
    ...
    def _get_client(self) -> Client:
        """Создаёт свежий клиент для каждого запроса (защита от stale connections)"""
        return create_client(self._url, self._key)

    def _safe_execute(self, request_builder):
        """Универсальная обёртка с retry при сетевых сбоях"""
        last_error = None
